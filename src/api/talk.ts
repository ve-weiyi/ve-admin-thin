...
import type { BatchResult, Talk, TalkDetailsDTO } from "./types";
...
/** 批量删除说说 */
export function deleteTalkByIdsApi(
  data: number[]
...
export function findTalkListApi(
  page: PageQuery
): Promise<IApiResponseData<PageResult<Talk[]>>> {
...
/** 分页获取说说详情列表 */
export function findTalkDetailsListApi(
  page: PageQuery
): Promise<IApiResponseData<PageResult<TalkDetailsDTO[]>>> {
...
/** 分页获取说说详情列表 */
export function findTalkDetailApi(
  id: number
): Promise<IApiResponseData<TalkDetailsDTO>> {
  return http.request<IApiResponseData<TalkDetailsDTO>>({
