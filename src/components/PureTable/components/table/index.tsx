...
import { Language, PureTableInstallOptions, PureTableProps, TableColumnScope } from "../../types"
import { ElConfigProvider, ElLoadingDirective, ElPagination, ElTable, ElTableColumn } from "element-plus"
import { debounce, isBoolean, isFunction, isString, nameHyphenate, useDark } from "@pureadmin/utils"
...
export default defineComponent({
  ...
  setup(props, { slots, attrs, emit, expose }) {
    ...
    let conditions = unref(pagination) && unref(pagination).currentPage && unref(pagination).pageSize
    ...
    let globalI18n = computed(() => {
      ...
      if (elLocale) {
        ...
        console.warn("@pureadmin/table：element-plus国际化文件未正确配置到vue-i18n中")
        return null
      }
    })
    ...
    let globalLocale = computed(() => {
      if (isString(defaultLocale)) {
        const _locale = [en, zhCn, zhTw].filter((i18) => i18.name === nameHyphenate(defaultLocale as string))[0]
        return _locale
      } else {
    ...
    let locales = computed(() => {
      ...
      if (isString(unref(locale))) {
        const _locale = [en, zhCn, zhTw].filter((i18) => i18.name === nameHyphenate(unref(locale) as string))[0]
        return _locale
      } else {
    ...
    const renderColumns = (columns: Record<string, any>, index: number) => {
      const { cellRenderer, slot, headerRenderer, headerSlot, hide, children, prop, ...args } = columns
    ...
    const debounceSetAdaptive = debounce(setAdaptive, unref(adaptiveConfig).timeout ?? 60)
        ...
        <>
          ...
          {conditions ? (
            <ElPagination
              ...
              layout={unref(pagination).layout ?? "total, sizes, prev, pager, next, jumper"}
      ...
      return (
        ...
        >
          {unref(globalI18n) || unref(globalLocale) || unref(locales) ? (
            <ElConfigProvider locale={unref(locales) ? unref(locales) : unref(globalI18n) || unref(globalLocale)}>
              {renderTable()}
            </ElConfigProvider>
          ) : (
            renderTable()
    ...
    return () => (ssr ? isClient.value && renderPureTable() : renderPureTable())
  },
})
