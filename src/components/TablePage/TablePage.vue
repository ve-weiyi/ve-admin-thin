      ...
      <div class="operation-container">
        ...
        <div class="flex-between align-right">
          <div class="flex-between">
            ...
            <el-button
              v-if="
                columnFieldsVisibility.filter(item => item === 'selection')
                  .length > 0
              "
              type="danger"
              size="default"
                    ...
                    <template #item="{ element, index }">
                      ...
                      >
                        ...
                        <el-checkbox
                          ...
                          :value="element.key"
                          @change="
                            value => handleCheckFieldChange(value, element)
                          "
                        >
                          <span
...
function handleCheckAllFieldChange(val: boolean) {
  isIndeterminate.value = false;
  // 改变按钮状态
  columnFieldsVisibility.value = val
    ? columnFields.value.map(column => column.key)
    : [];
  // 改变列显示状态
...
function handleCheckFieldChange(val: CheckboxValueType, element: any) {
  columnFields.value.filter(item => item.title === element.title)[0].hidden =
    !val;
