...
function handleCheckAllFieldChange(val: boolean) {
  ...
  isIndeterminate.value = false;
  // 改变按钮状态
  columnsVisibility.value = val
    ? dynamicColumns.value.map(column => column.title)
    : [];
  // 改变列显示状态
...
<template>
  ...
  <el-card v-loading="loading" shadow="never" class="table-bar">
    <div class="table-operation">
      ...
      <div class="flex items-center justify-around">
        <div style="margin-right: 1rem">
          <slot name="buttons" :size="size" :dynamicColumns="dynamicColumns" />
        </div>

        <el-tooltip
          v-if="props.tableRef?.hasChildren"
          content="展开表格"
          placement="top"
        >
          <ExpandIcon
    ...
    <slot name="default" :size="size" :dynamicColumns="dynamicColumns" />
  </el-card>
</template>
